...
hollow_knight_location = {}
hollow_knight_location.__index = hollow_knight_location
...
function hollow_knight_location.new(name)
    local self = setmetatable({}, hollow_knight_location)
    if name then
...
function hollow_knight_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = hollow_knight_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
function hollow_knight_location:connect_two_ways(exit, rule)
...
function hollow_knight_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function hollow_knight_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function hollow_knight_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function hollow_knight_location:connect_two_ways_stuck(exit, rule1, rule2)
...
function hollow_knight_location:accessibility()
...
-- 
function hollow_knight_location:discover(accessibility, keys)
...
entry_point = hollow_knight_location.new("entry_point")
